...
export default function AdminUserEditModal({ userToEdit, onClose }) {
    ...
    const [formData, setFormData] = useState({
        ...
        // newPassword: '', // Optional: for password reset
    });
    ...
    const handleSubmit = async (e) => {
        ...
        if (payload.activity_level === "") payload.activity_level = null; // Or default to sedentary if ''

        // Remove newPassword if empty, or backend will try to set an empty password
        // if (!payload.newPassword?.trim()) {
        //     delete payload.newPassword;
        // }

        // Don't send email if it hasn't changed from original
    ...
    return (
        <div className="fixed inset-0 bg-black/60 flex items-center justify-center z-50 p-4 overflow-y-auto">
            <form onSubmit={handleSubmit} className="bg-white dark:bg-gray-800 p-6 rounded-xl w-full max-w-lg space-y-4 shadow-2xl">
                <h2 className="text-xl font-semibold text-gray-800 dark:text-white text-center">Edit User: {userToEdit.name} (ID: {userToEdit.id})</h2>
                ...
                <div>
                    <label htmlFor="adminEditUserNewPassword" className="block text-sm font-medium text-gray-700 dark:text-gray-300">New Password (leave blank to keep current)</label>
