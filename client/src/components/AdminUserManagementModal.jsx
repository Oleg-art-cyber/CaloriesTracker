...
export default function AdminUserManagementModal({ isOpen, onClose }) {
    ...
    // Убираем создание authHeader здесь, будем создавать его по месту использования
    ...
    const fetchUsers = useCallback(async (page = 1, search = debouncedSearchTerm) => {
        if (!token) { // Если нет токена, не делаем запрос
            setError("Authentication required for user management.");
            setIsLoading(false);
            setUsers([]); // Очищаем пользователей
            setTotalUsers(0);
            setTotalPages(1);
            return;
        }
        ...
        setIsLoading(true);
        setError(null);
        // setSuccessMessage(''); // Можно сбрасывать здесь или при открытии модалки

        const localAuthHeader = { Authorization: `Bearer ${token}` }; // <--- Создаем authHeader здесь

        try {
            const response = await axios.get('/api/admin/users', {
                headers: localAuthHeader, // <--- Используем локальный authHeader
                params: {
                    page: page,
                    limit: usersPerPage,
            ...
            setUsers([]); // Очищаем при ошибке
            setTotalUsers(0);
            setTotalPages(1);
        } finally {
            setIsLoading(false);
        }
    }, [token, debouncedSearchTerm, usersPerPage]); // <--- Убрали authHeader из зависимостей

    useEffect(() => {
        if (isOpen) {
            // console.log("AdminUserManagementModal: Modal opened, fetching users.");
            setSuccessMessage(''); // Clear success message when modal opens/refetches
            setError(null); // Clear error message too
            fetchUsers(1, debouncedSearchTerm); // Fetch users for page 1 with current search
        } else {
            // Optional: Reset state when modal closes if needed
            // setSearchTerm(''); // This would clear search when modal closes
            // setSelectedProductId(''); // etc.
        }
    }, [isOpen, debouncedSearchTerm, fetchUsers]); // fetchUsers теперь стабилен, если token, debouncedSearchTerm, usersPerPage стабильны

    // ... (остальной код handleEditUser, handleDeleteUser, handleEditModalClose) ...
    // В handleDeleteUser и других вызовах axios также нужно создавать localAuthHeader:
    const handleDeleteUser = async (userIdToDelete, userName) => {
        ...
        try {
            setIsLoading(true);
            const localAuthHeader = { Authorization: `Bearer ${token}` }; // <--- ЗДЕСЬ
    ...
    return (
        // JSX остается тем же, что и в ответе #61
        // ...
